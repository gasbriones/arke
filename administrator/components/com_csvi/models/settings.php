 ...
 * @version 	$Id: settings.php 2380 2013-03-15 14:34:04Z RolandD $
 */
...
class CsviModelSettings extends JModelAdmin {
	...
	public function getResetSettings() {
		...
		$db = JFactory::getDbo();
		$db->setQuery('INSERT IGNORE INTO '.$db->qn('#__csvi_settings').' ('.$db->qn('id').', '.$db->qn('params').') VALUES ('.$db->q('1').', '.$db->q('').');');
		$db->query();
	 ...
	 * @since 		5.9
	 */
	public function getIcecatStats() {
		$stats = array();
		$db = JFactory::getDbo();
