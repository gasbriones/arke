 ...
 * @version 	$Id: userinfoimport.php 2273 2013-01-03 16:33:30Z RolandD $
 */
...
class CsviModelUserinfoimport extends CsviModelImportfile {
	...
	public function getProcessRecord() {
		...
		// We have a virtuemart_userinfo_id, do we have a virtuemart_user_id
			...
			$query->select('virtuemart_user_id')->from('#__virtuemart_userinfos')->where('virtuemart_userinfo_id = '.$db->quote($this->virtuemart_userinfo_id));
			$db->setQuery($query);
		...
		if (empty($this->virtuemart_userinfo_id)) {
			...
			if (!isset($this->virtuemart_user_id) && isset($this->email)) {
				...
				$query = $db->getQuery(true);
				$query->select('id')->from('#__users')->where('email = '.$db->Quote($this->email));
				$db->setQuery($query);
			...
			if ($this->virtuemart_user_id) {
				...
				$query->select('virtuemart_userinfo_id')
					...
					->where('address_type_name = '.$db->Quote($this->address_type_name));
				$db->setQuery($query);
		...
		if (isset($this->email)) {
			...
			if (!isset($this->group_id) && empty($this->usergroup_name)) {
				...
				$query = $db->getQuery(true);
				$query->select('id')->from('#__usergroups')->where($db->quoteName('title').' = '.$db->quote($this->usergroup_name));
				$db->setQuery($query);
		...
		if (!empty($this->country)) {
			$query = $db->getQuery(true);
			$query->select('virtuemart_country_id')->from('#__virtuemart_countries')->where('country_name = '.$db->quote($this->country));
			$db->setQuery($query);
			$this->virtuemart_country_id = $db->loadResult();
		}
		
		// Check for state ID
		if (!empty($this->state)) {
			$query = $db->getQuery(true);
			$query->select('virtuemart_state_id')->from('#__virtuemart_states')->where('state_name = '.$db->quote($this->state));
			$db->setQuery($query);
		...
		$csvilog->addDebug('VirtueMart user id: '.$this->virtuemart_user_id);
		$csvilog->addDebug('VirtueMart user info id: '.$this->_userinfos->virtuemart_userinfo_id);
