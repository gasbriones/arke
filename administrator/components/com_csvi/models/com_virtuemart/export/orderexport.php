 ...
 * @version 	$Id: orderexport.php 2351 2013-02-28 17:13:36Z RolandD $
 */
	...
	public function getStart() {
		...
		foreach ($export_fields as $column_id => $field) {
			switch ($field->field_name) {
				...
				case 'virtuemart_shipmentmethod_id':
					...
					break;
				case 'email':
					$userfields[] = $db->qn('user_info1.'.$field->field_name);
					break;
				case 'id':
				...
				case 'state_name':
					if ($address == 'BTST') {
						$userfields[] = 'COALESCE('.$db->qn('user_info2.virtuemart_state_id').', '.$db->qn('user_info1.virtuemart_state_id').') AS '.$db->qn('virtuemart_state_id');
					}
					else $userfields[] = $db->qn('user_info1.virtuemart_state_id');
					break;
				case 'country_2_code':
				case 'country_3_code':
				case 'country_name':
				case 'virtuemart_country_id':
					if ($address == 'BTST') {
						$userfields[] = 'COALESCE('.$db->qn('user_info2.virtuemart_country_id').', '.$db->qn('user_info1.virtuemart_country_id').') AS '.$db->qn('virtuemart_country_id');
					}
					else $userfields[] = $db->qn('user_info1.virtuemart_country_id');
					break;
				case 'user_currency':
				...
				case 'shipping_full_name':
					if ($address == 'BTST') {
						$userfields[] = $db->qn('user_info2.first_name', 'shipping_first_name');
						$userfields[] = $db->qn('user_info2.middle_name', 'shipping_middle_name');
						$userfields[] = $db->qn('user_info2.last_name', 'shipping_last_name');
					}
					break;
				...
				case 'virtuemart_product_id':
					$userfields[] = $db->qn('#__virtuemart_order_items.virtuemart_product_id');
					break;
				case 'custom':
				...
				default:
					...
					else $userfields[] = $db->qn($field->field_name);
					break;
			}
		}
		...
		if ($daterange != '') {
			...
			if ($orderdatestart) {
				...
				$selectors[] = $db->qn('#__virtuemart_orders').'.'.$db->qn('created_on').' >= '.$db->Quote($orderdate->toMySQL());
			}
			...
			if ($ordermdatestart) {
				...
				$selectors[] = $db->qn('#__virtuemart_orders').'.'.$db->qn('modified_on').' >= '.$db->Quote($ordermdate->toMySQL());
			}
		...
		if ($address) {
			switch (strtoupper($address)) {
				case 'BTST':
					...
					break;
				default:
					$selectors[] = 'user_info1.address_type = '.$db->q(strtoupper($address));
					break;
			}
		}
						...
						if ($field->process) {
							...
							switch ($fieldname) {
								...
								case 'username':
									...
									$query->where($db->qn('id').' = '.$record->virtuemart_user_id);
									$db->setQuery($query);
									$fieldvalue = CsviHelper::replaceValue($field->replace, $db->loadResult());
									// Check if we have any content otherwise use the default value
									if (strlen(trim($fieldvalue)) == 0) $fieldvalue = $field->default_value;
		...
		foreach ($export_fields as $column_id => $field) {
			...
			if ($process) {
				switch ($field->field_name) {
					...
					default:
						if ($address == 'BTST' && preg_match("/".$field->field_name."/i", join(",", array_keys($user_info_fields)))) {
							$fields[] = $db->qn('user_info1').'.'.$db->qn($field->field_name);
						}
