 ...
 * @version 	$Id: productexport.php 2039 2012-07-14 18:16:58Z RolandD $
 */
 ...
 * @subpackage 	Export
 */
class CsviModelYandexExport extends CsviModelExportfile {
	 ...
	 * Product export
	 *
	 * Exports product data to Yandex XML format
	 *
	 * @copyright
	 * @author		RolandD
	 * @todo
	...
	public function getStart() {
		...
		if (!is_null($csvidb->getErrorMsg())) {
			...
			if ($logcount > 0) {
				...
				$this->addExportContent($exportclass->categories($categories));
				...
				while ($record = $csvidb->getRow()) {
					...
					if ($template->get('export_file', 'general') == 'xml' || $template->get('export_file', 'general') == 'html') $this->addExportContent($exportclass->NodeStart($record->virtuemart_product_id));
