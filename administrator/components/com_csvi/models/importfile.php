 ...
 * @version		$Id: importfile.php 2381 2013-03-15 14:53:28Z RolandD $
 */
 ...
 * Import file Model
 */
class CsviModelImportfile extends JModelLegacy {
	...
	public function getHelperFiles() {
		...
		if (!array_key_exists('extension', $upload_parts)) return false;
			...
			switch ($data['options']['component']) {
				case 'mod_vm_cherry_picker':
					$helpers[] = 'com_virtuemart';
					break;
			}
	...
	public function getPrepareImport() {
		...
		if ($csvifile->validateFile()) {
			...
			if (!$csvifile->fp) {
				...
				// Unpublish any items if needed
				if (file_exists(JPATH_ADMINISTRATOR.'/components/com_csvi/helpers/'.$template->get('component', 'options').'.php')) {
					require_once JPATH_ADMINISTRATOR.'/components/com_csvi/helpers/'.$template->get('component', 'options').'.php';
					$component = $template->get('component', 'options');
					$helper = new $component;
					if (method_exists($helper, 'unpublishBeforeImport')) $helper->unpublishBeforeImport();
				}
	 ...
	 * @since 		3.0
	 */
	final public function getProcessData() {
		$jinput = JFactory::getApplication()->input;
		// Set some variables
		$data_preview = array();
		$processdata = true;
					...
					if (is_object($value) || is_array($value)) $this->_processSettings($value);
						...
						switch ($name) {
							...
							case 'source':
								...
								if ($value == 'fromupload') {
									$csvilog->addDebug(JText::_('COM_CSVI_IMPORT_UPLOAD_FILE_LABEL').': '.basename($jinput->get('csv_file', '', 'string')));
								}
								break;
							default:
								switch ($value) {
	...
	*/
	...
	private function _retrieveConfigFields($csvifile=false) {
		...
		// Use the fields assigned to the template
			...
			if (!empty($fields)) {
				...
				$csvilog->addDebug('COM_CSVI_USE_DATABASE_FOR_CONFIGURATION');
			}
			else {
				$csvilog->AddStats('incorrect', JText::_('COM_CSVI_NO_COLUMN_HEADERS_FOUND'));
				return false;
	 ...
	 */
	 protected function queryResult() {
	 	$db = JFactory::getDbo();
	 	return trim(substr($db->getQuery(), 0, strpos($db->getQuery(),' ')));
	 }
