 ...
 * @version 	$Id: log.php 2402 2013-03-28 17:04:25Z RolandD $
 */
	...
	public function getStoreLogResults() {
		...
		if ($settings->get('log.log_store', 1)) {
			...
			if (is_array($logresult) && !empty($logresult)) {
				$query = $db->getQuery(true)
					->insert($db->qn('#__csvi_log_details'))
					->columns(array($db->qn('id').','.$db->qn('log_id').','.$db->qn('line').','.$db->qn('description').','.$db->qn('result').','.$db->qn('status')));
				$row = 0;
				foreach ($logresult as $linenr => $result) {
					if (is_int($linenr)) {
						$row++;
						foreach ($result['status'] as $status => $stat) {
							$query->values($db->q('0').', '.$csvilog->getLogid().','.$linenr.','.$db->q(trim($stat['message'])).','.$db->q($stat['result']).','.$db->q($status));
						}
