 ...
 * @version 	$Id: maintenance.php 2400 2013-03-28 07:50:11Z RolandD $
 */
 ...
 * Maintenance Model
 */
class CsviModelMaintenance extends JModelList {

	/** @var string Model context string */
	protected $context = 'com_csvi.maintenance';
		...
		switch ($component) {
			...
			case 'com_ezrealty':
				...
				if (in_array($db->getPrefix().'ezrealty_images', $tables)) {
					$csvilog->setLinenumber($linenumber++);
					$q = "TRUNCATE TABLE `#__ezrealty_images`;";
					$db->setQuery($q);
					$csvilog->addDebug('Empty ezrealty images table', true);
					if ($db->query()) $csvilog->AddStats('empty', JText::_('COM_CSVI_EZREALTY_IMAGES_TABLE_HAS_BEEN_EMPTIED'));
					else $csvilog->AddStats('incorrect', JText::_('COM_CSVI_EZREALTY_IMAGES_TABLE_HAS_NOT_BEEN_EMPTIED'));
				}
	 ...
	 * @since 		3.0
	 */
	public function getVmExchangeRates() {
		$jinput = JFactory::getApplication()->input;
		$db = JFactory::getDbo();
		$csvilog = $jinput->get('csvilog', null, null);
		$linenumber = 1;
		...
		$q = "INSERT INTO ".$db->qn('#__csvi_currency')."(SELECT null, ".$db->qn('currency_code_3').", ".$db->qn('currency_exchange_rate')." FROM ".$db->qn('#__virtuemart_currencies')." WHERE ".$db->qn('currency_exchange_rate')." > 0)";
		$db->setQuery($q);
		if ($db->execute()) {
			$csvilog->AddStats('added', JText::_('COM_CSVI_VM_EXCHANGE_RATES_LOADED'));
		}
		else {
			$csvilog->AddStats('incorrect', JText::_('COM_CSVI_VM_EXCHANGE_RATES_NOT_LOADED'));
		}
	...
	public function getBackupVirtueMart() {
		...
		if ($fp) {
			...
			foreach ($tables as $table) {
				...
				$sqlstring .= "-- Table structure for table ".$db->qn($table)."\n\n";
				$sqlstring .= $tcreate[0]['Create Table'].";\n\n";

				// Check if there is any data in the table
				$q = "SELECT COUNT(*) FROM ".$db->qn($table);
				$db->setQuery($q);
				$count = $db->loadResult();

				if ($count > 0) {
					$sqlstring .= "-- Data for table ".$db->qn($table)."\n\n";
					// Get the field names
					$q = "SHOW COLUMNS FROM ".$db->qn($table);
					$db->setQuery($q);
					$fields = $db->loadObjectList();
					$sqlstring .= 'INSERT INTO '.$db->qn($table).' (';
					foreach ($fields as $field) {
						$sqlstring .= $db->qn($field->Field).',';
					}
	...
	public function getIcecatIndex() {
		...
		if ($username && $password) {
			...
			if ($load_index) {
				...
				$q = "TRUNCATE TABLE ".$db->qn('#__csvi_icecat_index');
				$db->setQuery($q);
				$db->query();

				// Load the local file into the database
				if (!$loadtype) {
					$q = "LOAD DATA LOCAL INFILE ".$db->q($icecat_index_file)."
						INTO TABLE ".$db->qn('#__csvi_icecat_index')."
			...
			if ($load_supplier) {
				...
				$q = "TRUNCATE TABLE ".$db->qn('#__csvi_icecat_suppliers');
				$db->setQuery($q);
				$db->query();
				...
				$q = "INSERT IGNORE INTO ".$db->qn('#__csvi_icecat_suppliers')."
	...
	public function getBackupTemplates() {
		...
		$csvilog = $jinput->get('csvilog', null, null);
		$linenumber = 1;
		$ids = $jinput->get('templates', array(), 'array');
		...
		if ($fp) {
			$query = $db->getQuery(true);
			$query->select($db->qn('id').", ".$db->qn('name').", ".$db->qn('settings').", ".$db->qn('process'))
				->from($db->qn('#__csvi_template_settings'));
			if (!empty($ids)) $query->where($db->qn('id').' IN ('.implode(',', $ids).')');
			$db->setQuery($query);
			...
			foreach ($templates as $template) {
				...
				$query->clear()
					...
					->where($db->qn('template_id').' = '.$template['id']);
				$db->setQuery($query);
				...
				$query->clear()
					->select($db->qn('x.field_id').','.$db->qn('r.name'))
					->from($db->qn('#__csvi_template_fields_replacement', 'x'))
					->leftJoin($db->qn('#__csvi_template_fields', 't').' ON '.$db->qn('x.field_id').' = '.$db->qn('t.id'))
					->leftJoin($db->qn('#__csvi_replacements', 'r').' ON '.$db->qn('x.replace_id').' = '.$db->qn('r.id'))
					->where($db->qn('x.field_id').' IN ('.implode(',', $fids).')')
					->order($db->qn('x.id'));
				$db->setQuery($query);
				$replacements = $db->loadObjectList();
				if (empty($replacements)) $replacements = array();
				$template['replace'] = json_encode($replacements);

				// Load the combine IDs for this template
				$query->clear()
					->select($db->qn('combine_id').','.$db->qn('field_id'))
					->from($db->qn('#__csvi_template_fields_combine'))
					->where($db->qn('field_id').' IN ('.implode(',', $fids).')')
					->order($db->qn('id'));
				$db->setQuery($query);
				$combines = $db->loadObjectList();
				if (empty($combines)) $combines = array();
				$template['combine'] = json_encode($combines);
	 ...
	 * @since 		3.0
	 */
	public function getRestoreTemplates($local=false, $file=null) {
		$jinput = JFactory::getApplication()->input;
		$db = JFactory::getDBO();
		$csvilog = $jinput->get('csvilog', null, null);
		$linenumber = 1;
		...
		if (empty($upload)) {
			...
			if ($local || is_uploaded_file($upload['tmp_name'])) {
				// Get some basic info
				...
				if (JFolder::create($folder)) {
					...
					if ($local) $result = JFile::copy($upload['tmp_name'], $folder.'/'.$upload['name']);
					else $result = JFile::upload($upload['tmp_name'], $folder.'/'.$upload['name']);
					if ($result) {
						...
						if ($fp) {
							while (($data = fgetcsv($fp, 0, ",")) !== FALSE) {
								$csvilog->setLinenumber($linenumber++);
								$db->setQuery("INSERT IGNORE INTO #__csvi_template_settings (".$db->qn('name').", ".$db->qn('settings').", ".$db->qn('process').")
											VALUES (".$db->q($data[0]).", ".$db->q($data[1]).", ".$db->q($data[2]).")");
								if ($db->query()) {
									...
									foreach ($fields as $key => $field) {
										$db->setQuery("INSERT IGNORE INTO #__csvi_template_fields (".$db->qn('template_id').", ".$db->qn('ordering').", ".$db->qn('field_name').", ".$db->qn('file_field_name').", ".$db->qn('column_header').", ".$db->qn('default_value').", ".$db->qn('process').", ".$db->qn('combine_char').", ".$db->qn('sort').", ".$db->qn('cdata').")
											VALUES (".$db->q($template_id).", ".$db->q($field->ordering).", ".$db->q($field->field_name).", ".$db->q($field->file_field_name).", ".$db->q($field->column_header).", ".$db->q($field->default_value).", ".$db->q($field->process).", ".$db->q($field->combine_char).", ".$db->q($field->sort).", ".$db->q($field->cdata).")");
										if ($db->query()) {
											$fields->$key->newid = $db->insertid();

										} // if
									} // foreach

									// Store the replacement and combine fields
									// Field is stored add the replacement link
									foreach ($replacements as $replacement) {
										if (isset($replacement->name)) {
											// Get the replacement ID
											$query = $db->getQuery(true);
											$query->select($db->qn('id'))->from($db->qn('#__csvi_replacements'))->where($db->qn('name').' = '.$db->q($replacement->name));
											$db->setQuery($query);
											$rid = $db->loadResult();
											if ($rid) {
												$fid = $replacement->field_id;
												// Store the replacemnt
												$db->setQuery("INSERT IGNORE INTO #__csvi_template_fields_replacement (".$db->qn('field_id').", ".$db->qn('replace_id').")
														VALUES (".$db->q($fields->$fid->newid).", ".$db->q($rid).")");
												$db->query();
											}
										}
										$rid = null;
									...
									foreach ($combines as $combine) {
										...
										$fid = $combine->field_id;
										if (isset($fields->$cid) && isset($fields->$fid)) {
											$db->setQuery("INSERT IGNORE INTO #__csvi_template_fields_combine (".$db->qn('field_id').", ".$db->qn('combine_id').")
													VALUES (".$db->q($fields->$fid->newid).", ".$db->q($fields->$cid->newid).")");
											$db->query();
										}
									} // foreach
								}
	...
	public function getUnpublishProductByCategory() {
		...
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		$query->select($db->qn('p').'.'.$db->qn('virtuemart_product_id'));
		$query->from($db->qn('#__virtuemart_products').' AS p');
		$query->innerJoin($db->qn('#__virtuemart_product_categories').' AS pc ON '.$db->qn('p').'.'.$db->qn('virtuemart_product_id').' = '.$db->qn('pc').'.'.$db->qn('virtuemart_product_id'));
		$query->innerJoin($db->qn('#__virtuemart_categories').' AS c ON '.$db->qn('pc').'.'.$db->qn('virtuemart_category_id').' = '.$db->qn('c').'.'.$db->qn('virtuemart_category_id'));
		$query->where($db->qn('p').'.'.$db->qn('published').' = '.$db->quote('1'));
		$query->where($db->qn('c').'.'.$db->qn('published').' = '.$db->quote('0'));
		...
		if (!empty($ids)) {
			// Unpublish the IDs
			$query = $db->getQuery(true);
			$query->update($db->qn('#__virtuemart_products'));
			$query->set($db->qn('published').' = '.$db->quote('0'));
			$query->where($db->qn('virtuemart_product_id').' IN ('.implode(',', $ids).')');
	...
	*/
	public function getSortCategories() {
		...
		if (!in_array($db->getPrefix().'virtuemart_categories_'.$language, $tables)) {
			...
			// Get all categories
			$query = $db->getQuery(true);
			$query->select('LOWER('.$db->qn('category_name').') AS '.$db->qn('category_name'));
			$query->select($db->qn('category_child_id').' AS '.$db->qn('cid'));
			$query->select($db->qn('category_parent_id').' AS '.$db->qn('pid'));
			$query->from($db->qn('#__virtuemart_categories').' AS '.$db->qn('c'));
			$query->leftJoin($db->qn('#__virtuemart_category_categories').' AS '.$db->qn('cc').' ON '.$db->qn('c').'.'.$db->qn('virtuemart_category_id').' = '.$db->qn('cc').'.'.$db->qn('category_child_id'));
			$query->leftJoin($db->qn('#__virtuemart_categories_'.$language).' AS '.$db->qn('cl').' ON '.$db->qn('cc').'.'.$db->qn('category_child_id').' = '.$db->qn('cl').'.'.$db->qn('virtuemart_category_id'));
			...
			if (count($records) > 0) {
				...
				foreach ($categories as $id => $category) {
					...
					foreach ($category as $category_id => $category_name) {
						...
						$query = $db->getQuery(true);
						$query->update($db->qn('#__virtuemart_categories'));
						$query->set($db->qn('ordering').' = '.$db->quote($listorder));
						$query->where($db->qn('virtuemart_category_id').' = '.$db->quote($category_id));
						$db->setQuery($query);
						$db->query();
	...
	public function getOperations() {
		...
		switch ($component) {
			case 'com_csvi':
				...
				$options .= '<option value="loadpatch">'.JText::_('COM_CSVI_PATCH_FILE_LABEL').'</option>';
			...
			case 'com_virtuemart':
				...
				$options .= '<option value="removeproductprices">'.JText::_('COM_CSVI_REMOVEPRODUCTPRICES_LABEL').'</option>';
				...
				$options .= '<option value="vmexchangerates">'.JText::_('COM_CSVI_VMEXCHANGERATES_LABEL').'</option>';
				break;
			case 'com_ezrealty':
				$options .= '<option value="">'.JText::_('COM_CSVI_MAKE_CHOICE').'</option>';
				$options .= '<option value="emptydatabase">'.JText::_('COM_CSVI_EMPTYDATABASE_LABEL').'</option>';
	...
	public function removeProductMediaLink() {
		...
		$db = JFactory::getDbo();
		$csvilog = $jinput->get('csvilog', null, null);
		$query = "TRUNCATE TABLE ".$db->qn('#__virtuemart_product_medias');
		$db->setQuery($query);
		if ($db->query()) {
	...
	public function loadPatch() {
		...
		$upload = JRequest::getVar('patch_file', '', 'files');
				...
				if (JFolder::create($folder)) {
					...
					if (JFile::upload($upload['tmp_name'], $folder.'/'.$upload['name'])) {
						...
						if (JArchive::extract($folder.'/'.$upload['name'], $folder)) {
							...
							if ($this->_processFolder($folder, $folder)) {
	 ...
	 * @since 		5.6
	 */
	private function _processFolder($folder, $base = null) {
		$jinput = JFactory::getApplication()->input;
		$csvilog = $jinput->get('csvilog', null, null);
		...
		foreach ($foundfiles as $ffkey => $ffname) {
			...
			if (is_dir($src)) {
				...
				if (JFile::copy($src, $destFile)) {
					$csvilog->AddStats('added', JText::sprintf('COM_CSVI_COPY_PATCHFILE', $src, $destFile));
				}
				else $csvilog->AddStats('incorrect', JText::sprintf('COM_CSVI_CANT_COPY_PATCHFILE', $src, $destFile));
			}
		}
	 ...
	 * @since 		5.9.5
	 */
	public function getRemoveProductPrices() {
		$jinput = JFactory::getApplication()->input;
		$csvilog = $jinput->get('csvilog', null, null);
		$db = JFactory::getDbo();

		$db->truncateTable('#__virtuemart_product_prices');
		$csvilog->AddStats('empty', JText::_('COM_CSVI_VM_PRICES_EMPTIED'));
	}
}