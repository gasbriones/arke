...
class TableVm_product_type_parameter extends JTable {
		...
		if($k) {
			...
			foreach ($dbfields as $name => $details) {
				if (!empty($this->$name)
					...
					&& $name != 'paramater_name') {
					$q .= $db->qn($name)." = ".$db->quote($this->$name).', ';
				}
			}
			$q = substr($q, 0, -2);
	...
	* Delete a product type parameter from the #__vm_product_type_<id> table
	...
	private function DeleteProductTypeParam() {
		$db = JFactory::getDbo();
		$q = 'ALTER TABLE '.$db->qn( '#__vm_product_type_'.$this->product_type_id ).
			' DROP COLUMN '.$db->qn($this->parameter_name);
		$db->setQuery($q);
		return $db->query();
	}
}