...
class TableShipmentmethods_lang extends JTable {
	...
	public function __construct($db) {
		...
		if ($template->get('operation', 'options') == 'shippingrateimport') {
		...
		parent::__construct('#__virtuemart_shipmentmethods_'.$lang, 'virtuemart_shipmentmethod_id', $db );
	}
	...
	public function check($create = true) {
		...
		if (!empty($this->virtuemart_shipmentmethod_id)) {
			...
			$id = $db->loadResult();
			if ($id > 0) {
				$csvilog->addDebug('COM_CSVI_DEBUG_SHIPMENTMETHOD_EXISTS', true);
				...
				if ($create) {
					...
					if ($db->query()) {
						...
						$csvilog->addDebug('COM_CSVI_DEBUG_SHIPMENTMETHOD_NOT_EXISTS', true);
						return false;
					}
				}
				else {
	...
	private function _validateSlug() {
		...
		$db->setQuery($query);
		$slugs = $db->loadResult();
		$csvilog->addDebug('COM_CSVI_CHECK_SHIPMENTMETHOD_SLUG', true);
		if ($slugs > 0) {
