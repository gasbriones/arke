 ...
 * Export Controller
 */
class CsviControllerExportfile extends JControllerLegacy {
	...
	public function process() {
		...
		if ($model->getPrepareExport()) {
			...
			$overridefile = JPATH_BASE.'/templates/'.$app->getTemplate().'/html/com_csvi/models/'.$template->get('component', 'options').'/export/'.$template->get('operation', 'options').'.php';
			...
			$this->setRedirect('index.php?option=com_csvi&view=process&template_id='.$jinput->get('template_id', 0, 'int'), JText::_('COM_CSVI_ERROR_EXPORT_PREP'), 'error');
		}
	}
}