 ...
 * @version 	$Id: com_virtuemart.php 2383 2013-03-17 09:08:01Z RolandD $
 */
...
class Com_VirtueMart {
		...
		switch ($update_based_on) {
			case 'product_sku':
				...
				if ($product_id) {
					...
					if ($product_sku) {
						...
						$query->where($db->qn('product_sku').' = '.$db->q($product_sku));
						$db->setQuery($query);
			...
			case 'product_mpn':
				...
				if ($product_mpn) {
					...
					$query->select($db->qn('virtuemart_product_id'));
					$query->from($db->qn('#__virtuemart_products'));
					$query->where($db->qn($mpn_column)." = ".$db->q($product_mpn));
					$db->setQuery($query);
			...
			case 'product_child_sku':
				...
				if ($product_sku && $product_parent_sku) {
					...
					$query->select($db->qn('virtuemart_product_id'));
					$query->from($db->qn('#__virtuemart_products'));
					$query->where($db->qn('product_sku').' = '.$db->Quote($product_parent_sku));
					$db->setQuery($query);
					...
					$query->from($db->qn('#__virtuemart_products'));
					$query->where($db->qn('product_sku').' = '.$db->q($product_sku));
					$query->where($db->qn('product_parent_id').' = '.$product_parent_id);
					$db->setQuery($query);
	...
	public function getShopperGroupId($shopper_group_name) {
		...
		$query->from($db->qn('#__virtuemart_shoppergroups'));
		$query->where($db->qn('shopper_group_name').' = '.$db->q($shopper_group_name));
		$db->setQuery($query);
	...
	public function getRelatedId($type='R') {
		if (!$this->_related_id) {
			...
			$query->where('field_type = '.$db->q($type));
			$db->setQuery($query);
	...
	*/
	...
	public function getOrderItemProduct() {
		...
		$q = "SELECT DISTINCT order_item_sku AS product_sku, order_item_name AS product_name
				FROM #__virtuemart_order_items o
				WHERE (o.order_item_sku LIKE ".$db->q('%'.$filter.'%')."
					OR o.order_item_name LIKE ".$db->q('%'.$filter.'%').")
				ORDER BY order_item_name
				LIMIT 10;";
		$db->setQuery($q);
		return $db->loadObjectList();
	...
	public function unpublishBeforeImport() {
		...
		$csvilog = $jinput->get('csvilog', null, null);
		$template = $jinput->get('template', null, null);
		if ($template->get('unpublish_before_import', 'product', 0)) {
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$query->update('#__virtuemart_products')->set('published = 0');
			$db->setQuery($query);
