 ...
 * @version 	$Id: csvi.php 2389 2013-03-21 09:03:25Z RolandD $
 */
...
class CsviHelper {
	 ...
	 * @since 		3.0
	 */
	public function replaceValue($ids, $value) {
		static $replacements;
		...
		if (is_array($ids)) {
			foreach ($ids as $id) {
				if (!isset($this->replacements[$id])) {
					...
					$query->select($db->qn('findtext'))
						->select($db->qn('replacetext'))
						->select($db->qn('multivalue'))
						->select($db->qn('method'))
						->from('#__csvi_replacements')
						->where('id = '.$db->q($id));
					$db->setQuery($query);
				...
				if (empty($replace)) return $value;
					...
					if ($replace->multivalue) {
						$findtext = explode('|', $replace->findtext);
						$replacetext = explode('|', $replace->replacetext);
					}
					else {
	 ...
	 * @since 		4.0
	 */
	public function getComponents($filters=null) {
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		$query->select("component AS value, CONCAT('COM_CSVI_', UPPER(component)) AS text");
		...
		if ($filters) {
			foreach ($filters as $name => $value) {
				$query->where($db->qn($name).' = '.$db->q($value));
			}
		}

		$query->group('component');
		$query->order($db->qn('#__csvi_template_types.ordering'));
		$db->setQuery($query);
	...
	public function getButtons() {
		...
		$cpanel_images->analyzer = $this->CpanelButton('csvi_analyzer_48.png', 'index.php?option=com_csvi&view=analyzer', 'COM_CSVI_ANALYZE');
		...
		$cpanel_images->maps = $this->CpanelButton('csvi_order_48.png', 'index.php?option=com_csvi&view=maps', 'COM_CSVI_MAPS');
		$cpanel_images->tasks = $this->CpanelButton('csvi_template_48.png', 'index.php?option=com_csvi&view=templatetypes', 'COM_CSVI_TEMPLATETYPES');
	...
	public static function addSubmenu($vName) {
		JHtmlSidebar::addEntry(
				JText::_('COM_CSVI_PROCESS'),
				'index.php?option=com_csvi&view=process',
				$vName == 'process'
		);

		JHtmlSidebar::addEntry(
				JText::_('COM_CSVI_REPLACEMENTS'),
				'index.php?option=com_csvi&view=replacements',
				$vName == 'replacements'
		);

		JHtmlSidebar::addEntry(
				JText::_('COM_CSVI_MAPS'),
				'index.php?option=com_csvi&view=maps',
				$vName == 'maps'
		);

		JHtmlSidebar::addEntry(
				JText::_('COM_CSVI_LOG'),
				'index.php?option=com_csvi&view=log',
				$vName == 'log'
		);

		JHtmlSidebar::addEntry(
				JText::_('COM_CSVI_MAINTENANCE'),
				'index.php?option=com_csvi&view=maintenance',
				$vName == 'maintenance'
		);

		JHtmlSidebar::addEntry(
				JText::_('COM_CSVI_AVAILABLE_FIELDS'),
				'index.php?option=com_csvi&view=availablefields',
				$vName == 'availablefields'
		);

		JHtmlSidebar::addEntry(
				JText::_('COM_CSVI_TEMPLATETYPES'),
				'index.php?option=com_csvi&view=templatetypes',
				$vName == 'templatetypes'
		);

		JHtmlSidebar::addEntry(
				JText::_('COM_CSVI_ANALYZE'),
				'index.php?option=com_csvi&view=analyzer',
				$vName == 'analyzer'
		);

		JHtmlSidebar::addEntry(
				JText::_('COM_CSVI_SETTINGS'),
				'index.php?option=com_csvi&view=settings',
				$vName == 'settings'
		);

		JHtmlSidebar::addEntry(
				JText::_('COM_CSVI_ABOUT'),
				'index.php?option=com_csvi&view=about',
				$vName == 'about'
		);

		JHtmlSidebar::addEntry(
				JText::_('COM_CSVI_INSTALL'),
				'index.php?option=com_csvi&view=install',
				$vName == 'install'
		);
	}
