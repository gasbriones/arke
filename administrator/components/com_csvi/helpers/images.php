 ...
 * @version 	$Id: images.php 2401 2013-03-28 16:52:09Z RolandD $
 */
	...
	*/
	...
	public function findMimeType($filename, $remote=false) {
		...
		if (JFile::exists($filename) || $remote) {
			if ($remote) $filename = str_ireplace(' ', '%20', $filename);
	...
	private function LoadMimeTypes() {
		...
		$this->_mime_types[86]['signature'] = 'FFD8FFEE';
		$this->_mime_types[87]['signature'] = 'FFD8FFE1';
		$this->_mime_types[88]['signature'] = 'FFD8FFDB';
		$this->_mime_types[89]['signature'] = '474946383961';
		...
		$this->_mime_types[88]['extension'] = '.jpeg;.jpe;.jpg';
		$this->_mime_types[89]['extension'] = '.gif';
		...
		$this->_mime_types[88]['description'] = 'JPG Graphic File';
		$this->_mime_types[89]['description'] = 'GIF 89A';
		...
		$this->_mime_types[88]['mime_type'] = 'image/jpeg';
		$this->_mime_types[89]['mime_type'] = 'image/gif';
	}
	...
	private function NewImgSave($new_img) {
		...
		$csvilog->addDebug('File out extension: '.$this->file_out_extension);
		$csvilog->addDebug('File out: '.$this->file_out);
		switch(strtolower($this->file_out_extension)) {
			...
			case "png":
				...
				break;
			default:
				$csvilog->addDebug('COM_CSVI_NO_FILE_EXTENSION');
				return false;
				break;
		}
	}
	 ...
	 * @param		string	$path	the destination location of the image including trailing /
	...
	public function processImage($name, $output_path, $output_name=null) {
		...
		if ($this->_imagedata['isremote']) {
			...
			$csvilog->AddStats('incorrect', JText::sprintf('COM_CSVI_DEBUG_FILE_NOT_FOUND', $this->_imagedata['name']));
		...
		if ($this->_imagedata['isimage']) {
			...
			if ($this->_imagedata['isremote'] && $template->get('save_images_on_server', 'image') && !$this->_imagedata['convert']) {
				// Sanitize filename
				...
				if (JFile::exists($to) && !$template->get('redownload_external_image', 'image')) {
					JFile::delete($from);
				}
				else {
					if ($from !== $to) {
			...
			if ($this->_imagedata['exists']) {
				$csvilog->addDebug(JText::sprintf('COM_CSVI_DEBUG_FILE_IS_NOT_IMAGE', $name));
	...
	private function _cleanupImage() {
		...
		if ($this->_imagedata['isremote'] && $template->get('save_images_on_server', 'image')) {
			...
			$csvilog->addDebug('Curfile: '.$curfile);
			if (!JFile::exists($curfile) || (JFile::exists($curfile) && $template->get('redownload_external_image', 'image'))) {
		...
		if (isset($mime_details[0])) $type = $mime_details[0];
		if (isset($mime_details[1])) $ext = $mime_details[1];
		...
		if ($convert_type != 'none' && $convert_type != $this->_imagedata['extension']) {
			...
			if ($template->get('auto_generate_image_name', 'image', false)) {
				$this->_imagedata['output_name'] = JFile::stripExt(basename($this->_imagedata['output_name'])).'.'.$convert_type;
			}
			else $this->_imagedata['output_name'] = JFile::stripExt(basename($this->_imagedata['name'])).'.'.$convert_type;
	...
	private function _convertImage() {
		...
		if ($this->_imagedata['convert'] && ($this->_imagedata['isremote'] && $template->get('save_images_on_server', 'image'))) {
	...
	private function _imageTypeCheck() {
		...
		$output_ext = JFile::getExt($this->_imagedata['output_name']);
		if ($output_ext == 'jpg') $mime_ext = 'jpeg';
		else $mime_ext = $output_ext;
	...
	public function addWatermark($imagename) {
		...
		$template = $jinput->get('template', null, null);
		$csvilog = $jinput->get('csvilog', null, null);
		$watermark = JPATH_SITE.'/'.$template->get('full_watermark_image', 'image');
		...
		if (file_exists($imagename) && file_exists($watermark)) {
			...
			$csvilog->addDebug('Mime image:'.$mime_image);
			$image = $this->_createImage($mime_image, $imagename);
			$ext = JFile::getExt($imagename);

			if ($image) {
				$mime_stamp = $this->findMimeType($watermark);
				$csvilog->addDebug('Mime stamp:'.$mime_stamp);
				$stamp = $this->_createImage($mime_stamp, $watermark);

				// Set the margins for the stamp and get the height/width of the stamp image
				$marge_right = $template->get('full_watermark_right', 'image');
				$marge_bottom = $template->get('full_watermark_bottom', 'image');
				$sx = imagesx($stamp);
				$sy = imagesy($stamp);
				...
				switch($ext) {
					case "gif":
						return imagegif($image, $imagename);
						break;
					case "jpg":
					case "jpeg":
						return imagejpeg($image, $imagename, 100);
						break;
					case "png":
						return imagepng($image, $imagename);
						break;
					default:
				...
				$csvilog->addDebug('COM_CSVI_CANNOT_CREATE_WATERMARK_FILE');
			}
		}
		else {
			$csvilog->addDebug('COM_CSVI_FILES_DONT_EXIST');
		}
	}
	 ...
	 * @since 		4.2
	 */
	private function _createImage($mime_type, $imagename) {
		$image = null;
		switch ($mime_type) {
			case 'image/gif':
				if( function_exists('imagecreatefromgif') ) {
