 ...
 * @version 	$Id: icecat.php 2355 2013-03-01 16:51:40Z RolandD $
 */
...
class IcecatHelper {
	...
	private $_featureids = array();
	private $_featurenames = array();
	...
	public function getData($mpn, $mf_name) {
		...
		$update_fields = $template->get('icecat_update_fields', 'product', array(), 'array');
			...
			if ($this->_setupXmlParser()) {
				...
				if ($this->_data) {
					...
					foreach ($update_fields as $fieldname) {
						if (array_key_exists($fieldname, $this->_csvi_data)) {
							unset($this->_csvi_data[$fieldname]);
						}
					}
	...
	private function _getIcecatUrl($mpn, $mf_name) {
		...
		$query->select('product_id')
			...
			->leftJoin($db->quoteName('#__csvi_icecat_suppliers').' AS s ON s.supplier_id = i.supplier_id')
			->where('i.'.$db->quoteName('prod_id').' = '.$db->q($mpn))
			->where('s.'.$db->quoteName('supplier_name').' = '.$db->q($mf_name));
		$db->setQuery($query);
		$csvilog->addDebug('COM_CSVI_FIND_ICECAT_ID', true);
		$icecat_id = $db->loadResult();

		// See if we have a match, otherwise try to search more liberal
		if (!$icecat_id && $template->get('similar_sku', 'product', false)) {
			$query = $db->getQuery(true);
			$query->select('product_id')
			->from($db->quoteName('#__csvi_icecat_index').' AS  i')
			->leftJoin($db->quoteName('#__csvi_icecat_suppliers').' AS s ON s.supplier_id = i.supplier_id')
			->where('i.'.$db->quoteName('prod_id').' LIKE '.$db->quote($mpn.'%'))
			->where('s.'.$db->quoteName('supplier_name').' = '.$db->Quote($mf_name));
			$db->setQuery($query);
			$csvilog->addDebug('COM_CSVI_FIND_ICECAT_ID', true);
			$icecat_id = $db->loadResult();
			...
			if (!$icecat_id) {
				...
				->from($db->quoteName('#__csvi_icecat_index').' AS  i')
				->leftJoin($db->quoteName('#__csvi_icecat_suppliers').' AS s ON s.supplier_id = i.supplier_id')
				->where('i.'.$db->quoteName('m_prod_id').' = '.$db->quote($mpn))
				->where('s.'.$db->quoteName('supplier_name').' = '.$db->Quote($mf_name));
				$db->setQuery($query);
				$csvilog->addDebug('COM_CSVI_FIND_ICECAT_ID', true);
				$icecat_id = $db->loadResult();
		...
		switch ($tagname) {
			...
			case 'name':
				switch ($parent_tag) {
					case 'category':
						...
						break;
					case 'featuregroup':
						$this->_featurenames[$this->_featureid] = $attribs['VALUE'];
						break;
					case 'feature':
						$csvilog->addDebug(JText::sprintf('COM_CSVI_ICECAT_FOUND_FEATURE', $attribs['VALUE']));
						if (isset($this->_featurenames[$this->_categoryfeaturegroup_id])) {
							$this->_csvi_data['features'][$this->_categoryfeaturegroup_id][$this->_featurenames[$this->_categoryfeaturegroup_id]][$attribs['VALUE']] = str_replace('\n', '<br />', $this->_presentationvalue);
						}
			...
			case 'productpicture':
				if (!empty($attribs)) {
					...
					if (isset($this->_csvi_data['file_url_thumb']))	$this->_csvi_data['file_url_thumb'] .= '|'.$attribs['THUMBPIC'];
					else $this->_csvi_data['file_url_thumb'] = $attribs['THUMBPIC'];
					if (isset($this->_csvi_data['file_url'])) $this->_csvi_data['file_url'] .= '|'.$attribs['PIC'];
					else $this->_csvi_data['file_url'] = $attribs['PIC'];
				}
				break;
			...
			case 'productdescription':
				...
				if (!empty($attribs['MANUALPDFURL'])) {
					if (isset($this->_csvi_data['file_url'])) $this->_csvi_data['file_url'] .= '|'.$attribs['MANUALPDFURL'];
					else $this->_csvi_data['file_url'] = $attribs['MANUALPDFURL'];

					if (isset($this->_csvi_data['file_url_thumb'])) $this->_csvi_data['file_url'] .= '|';
					else $this->_csvi_data['file_url_thumb'] = '';
				}
				if (!empty($attribs['PDFURL'])) {
					if (isset($this->_csvi_data['file_url'])) $this->_csvi_data['file_url'] .= '|'.$attribs['PDFURL'];
					else $this->_csvi_data['file_url'] = $attribs['PDFURL'];
