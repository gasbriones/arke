 ...
 * @version 	$Id: csv.php 2350 2013-02-28 15:04:23Z RolandD $
 */
...
class CsvFile extends CsviFile {
	...
	public function loadColumnHeaders() {
		...
		// 3. Read the line
		$jinput->set('columnheaders', $this->ReadNextLine(true));
	...
	public function readNextLine($headers=false) {
		...
		if (!empty($this->_field_delimiter)) {
			...
			if ($csvdata) {
				...
				if ($headers) return $csvdata;
				else {
					// Add the data to the fields
					foreach ($csvdata as $key => $value) {
						if (isset($columnheaders[$key])) $csvifields->set($columnheaders[$key], $value);
					}
					return true;
				}
			}
			...
			$csvilog->AddStats('incorrect', JText::_('COM_CSVI_NO_FIELD_DELIMITER_FOUND'));
			return false;
		}
	}
