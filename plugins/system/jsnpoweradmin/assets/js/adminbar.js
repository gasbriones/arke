...
({
	...
	createUI: function ()
	{
		...
		this.bodyWrapper.setStyles({
			...
			paddingTop 		: parseInt(this.body.getStyle('padding-top')) + parseInt(this.body.getStyle('margin-top')) + 4,
		...
		try {
			...
			$(jsnMenu).getElements('>.has-child>ul').addClass(this.dropdown_class);
			$(jsnMenu).getElements('>.has-child>ul li.has-child').addClass(this.submenu_dropdown_class);
			$(jsnMenu).getElements('>.has-child>ul>li>ul').addClass(this.dropdown_class);
...
//		});
	...
	getPageTitle: function (urlparams) {		
		...
		if (!pagetitle){
			if ($$('body').getElement('.pagetitle') == null) {
				return '';
			}
			pagetitle = $$('body').getElement('.pagetitle').textContent;
