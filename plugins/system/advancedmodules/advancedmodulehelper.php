		...
		if ($module->published)
		{
			...
			)
			{
				...
				else
				{
					...
					{
						$mirror_params = plgSystemAdvancedModulesGetAdvancedParams($mirror_moduleid);
						$mirror_params = $parameters->getParams($mirror_params, $xmlfile_assignments);
					}
				}

				// Only copy assignmnet settings from mirrored module
				foreach ($mirror_params as $key => $val)
				{
					if (substr($key, 0, 9) != 'assignto_')
					{
						continue;
					}
					$module->advancedparams->$key = $val;
				}
			}
		}
